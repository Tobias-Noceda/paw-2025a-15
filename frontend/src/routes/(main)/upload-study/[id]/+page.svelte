<script lang="ts">
					...
					props: {
						...
						onclick: (selected: boolean) => {
							if (selected) {
                                ...
                                selectedDoctorsSelfs = selectedDoctorsSelfs.filter(self => self !== doctor.links.self);
                            }
						},
    ...
    const handleSubmit = async () => {
        ...
        const location = await createStudy(
            data.patient.links.studies.resolved!,
            {
                comment: extraData,
                studyDate: fileDate,
...
<div class="flex justify-center">
	<div class="page-container-division bg-transparent! h-fit max-h-full! flex flex-col w-160 gap-2.5 select-text">
        <div class="page-card flex flex-col gap-5 w-full h-fit max-h-full! overflow-y-auto">
            <h1 class="w-full text-center text-[24px] font-bold mb-2.5">{m['new_file.title']({ patientName: data.patient.name })}</h1>
            <div class="flex flex-col gap-3">
                <p class="text-line text-primaryText font-bold select-none">{m['new_file.labels.files']()}:</p>
                <div class="flex gap-5 items-center">
                    ...
                    {:else}
                        <ul class="list-disc list-inside text-ellipsis overflow-hidden">
                            ...
                            {/each}
                            {#if files.length > 2}
                                <p class="truncate text-gray-400">{m['new_file.more_files']({ count: files.length - 2 })}</p>
                            {/if}
                        </ul>
                    {/if}
                </div>
